...
Xmake 还尝试解决 C++ 的又一大痛点：**构建系统语法混乱**。尽管 *CMake* 成了事实上的标准，但很多人嫌弃 CMake，主要是 CMake 自创的语法太丑陋了。因此有时人们会使用别的工具，但别的工具语法也非常复杂。Xmake 没有弄出新的语法，它使用 lua 来编写构建文件。lua 语法很简单，即使没有相关基础也能快速使用。
...
Xmake 还还还试图解决 C++ 的又又又一大痛点 ~~C++ 的痛点实在是太多了~~ ：**工具链不统一**。C++ 的实现非常多，除了常见的 GCC、CLANG、MSVC 外，还有 MinGW、TinyC、SDCC 等。XMake 可以通过简单的命令快速切换工具链进行构建，无需设置手动非常复杂的环境变量。

当然 Xmake 也有缺点。现代工具的共同点就是年轻，尽管理念先进，但相应的生态并不成熟，想要完全取代老牌构建工具 CMake，可能还需要很长的时间。新的、简单的、没有复杂依赖的项目，可以试试使用 Xmake。
...
是的就是这么简单，现在运行 `xmake` 就会尝试以新的工具链构建项目，无需更改构建文件，无需定义新的环境变量。
