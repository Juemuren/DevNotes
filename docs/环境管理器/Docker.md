...
Docker 代表了容器技术。所谓容器，就是个把应用程序及其运行环境打包在一起的东西。你可以把容器给别人使用，也可以使用别人的容器。

可以直接在容器里进行开发，这样能够省去所有配置开发环境的麻烦。
...
2. 在 Windows 里安装 Docker，并通过配置让 `docker` 连接到 WSL 里的 `dockerd`
