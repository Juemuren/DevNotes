...
- **科学计算** 使用 Conda/Pixi 管理环境。科学计算项目是典型的 *混合栈*，比如一个 Python 库通常要用 C++ 编译的二进制文件加速，并且还可能依赖特定版本的 CUDA 运行时。Conda/Pixi 对于这种多语言混合环境处理得最好。
- **应用开发** 使用 Mise 管理环境。实际的应用项目普遍存在 *多个独立的运行时*，比如前端开发环境通常需要 NodeJS，后端通常需要 Go，一个项目甚至可能有多个运行时各不相同前端或后端。Mise 非常适合应对这种场景。
