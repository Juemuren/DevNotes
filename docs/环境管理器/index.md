...
- **科学计算** 使用 Conda 管理环境。科学计算项目是 *混合栈* 的典型，比如一个 Python 库要用 C++ 编译的二进制文件加速，并且还可能依赖特定 CUDA 版本的运行时。Conda 对于这种多语言混合环境处理得最好。
- **应用开发** 使用 Mise 管理环境。实际的应用项目普遍存在 *多个独立的运行时*，比如前端开发环境需要 NodeJS，后端需要 Go，甚至可能有多个前端或后端，且其运行时版本各不相同。Mise 非常适合应对这种场景。
