...
- 语法：规定了哪些代码是合法的，比如语言的关键字有哪、，运算符怎么使用、正确的语句长什么样等。
- 语义：确定了合法代码的含义
- 标准库：语言官方提供的一系列核心功能模块的接口和行为定义
...
- 先通过**编译器**把源代码变成字节码，然后在**虚拟机**中运行字节码（**虚拟机**其实和**解释器**很类似，但习惯把执行源代码的叫作**解释器**，运行字节码的叫作**虚拟机**），比如 Java/C#

其中有些实现方式还有变种，比如

- 有些使用**解释器**和**虚拟机**的语言会引入 `JIT` 技术，可以把反复执行的热点代码动态地编译成本地机器码从而提高运行速度。这需要语言实现一个 **JIT 编译器**，典型例子是 Julia
...
有些语言不只一种实现。比如 C++ 有非常多的编译器，同时还有解释器；比如 C# 不仅可以编译为字节码然后在虚拟机中运行，还可以直接编译成不依赖虚拟机的本地机器码。

### 编程语言的生态

对于现代的编程语言，仅有规范和实现是不够的，生态系统也是编程语言重要的组成部分，通常包括

- 工具链：各种开发工具，比如调试器、构建系统、包管理器、IDE 插件等
...
>
> 不过也有部分语言相关的知识是难以通过只看文档掌握的，比如应该选择什么工具/库/框架、最佳实践是什么样的、应该采用什么设计模式等。有些东西和具体的领域有关，只有真正上手写了项目，才会慢慢学会这些东西。
>
> 每个开发者都会有自己的喜好，无论何时何地，讨论哪个语言更好都会引起争论。但语言事实上只是工具，我们用工具解决问题，而不是用工具异化自己。我很喜欢尝试不同的语言，这让我真实地感受到了它们之间的差异，也能够更加客观地评价它们各自的优劣。你不应该根据所谓的好坏和流行度来选择语言，而应该根据自己的需要去选择合适的语言。不要畏惧新东西，因为我们只是使用语言的子集来解决实际遇到的问题，而不是掌握这门语言的全部。
...
- [CUDA](CUDA.md) 严格地说 CUDA 不是一门语言，而是由 NVIDIA 推出的一个异构编程模型，不过也可以把它简单理解为一个 C++ 方言。CUDA 拓展了 C++ 的语法，可以声明哪些代码应该在 GPU 上运行。编译时，NVCC 会把 GPU 部分的代码分离出来，编译成特定 GPU 架构的二进制指令序列，然后作为字符数组嵌入 C++ 代码中，再让 C++ 编译器编译并链接到 CUDA 运行时库，从而得到完整的可执行程序。
...
- [Go](Go.md) 由 Google 设计并开源的语言，能够简单地编写并发程序，可作为大多数后端项目的首选，很多云原生基础设施也都是用 Go 写的。
...
- Lua 因为解释器非常轻量而经常被其它应用拿来解析脚本。比如 neovim 的配置文件、xmake 的构建描述、pandoc 的过滤器等都使用 Lua 进行解析。
