...
如果使用 MSYS 获取工具链，请先完成 [MSYS 的安装](../环境管理/MSYS.md#安装)，然后通过如下命令安装编译器和调试器
...
VS 为了不污染系统环境，需要进入编译环境后能使用编译器。和 MSYS 一样，我不推荐去修改系统 PATH 变量，而是使用官方提供的脚本进入编译环境。这个脚本一般在 VS 安装目录的 `Common7\Tools\Launch-VsDevShell.ps1` 下。当然，除了使用脚本，官方还提供了快捷方式并且配置好了 `Windows Terminal`。不过我个人不太喜欢这些东西
...
- [Vcpkg](../包管理/Vcpkg.md)
- conan
- [Xrepo](../包管理/Xmake.md) Xmake 自带的包管理器，因此放到了 Xmake 的章节中

### 构建系统

- make
- CMake 事实上的标准，但语法极度丑陋
- [Xmake](../包管理/Xmake.md) 现代构建工具，结合了包管理、项目管理等功能，尝试解决 C++ 的诸多痛点。
