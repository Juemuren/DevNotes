...
> [!Note]- 前端和后端
>
> 现代 *Web* 应用通常都会使用 **前后端分离** 的设计：前端实现浏览器里运行的那部分，后端实现服务器里运行的那部分。但我不会把后端开发归类为 Web 开发，因为后端实现的 API 也可以用于非 Web 应用。
>
> 另外，我其实不是很喜欢 *前端* 和 *后端* 这种说法，这两个词在计算机相关领域已经被严重滥用了。暂且不说编译器、集成电路等地方也在使用前后端这种术语，单单 Web 方面前端一词就非常模糊：有时前端几乎就是 Web 的同义词，有时用来指代 UI，有时含义又更接近客户端。我喜欢更精确的、不会引起歧义的说法：开发 *Web* 应用就叫 *Web 开发*，开发 *API* 就叫 *API 开发*。不过考虑到社区更习惯使用 *前后端*，因此本文会保留这种用法。
