...
> [!Note]- 前端和后端
>
> 现代 *Web* 应用通常都会使用 **前后端分离** 的设计：前端实现浏览器里运行的那部分，后端实现服务器里运行的那部分。但我不会把后端开发归类为 Web 开发，因为后端实现的 API 也可以用于非 Web 应用。我甚至不认为后端是应用的一部分，因为严格地说只有运行在浏览器里的那部分才叫 *Web 应用*，而运行在服务器里的应用只是给 *Web 应用* 提供了服务。
>
> 另外，我其实不是很喜欢 *前端* 和 *后端* 这种说法，因为这两个词在计算机相关领域已经被严重滥用了。暂且不说编译器、集成电路等地方也在使用前后端这种术语，单单 Web 方面前端一词就非常模糊：有时前端几乎就是 Web 的同义词，有时用来指代 UI，有时含义又更接近客户端。当然这可能是因为 Web 技术发展得太快，而人们改变习惯用语需要时间，才导致前端一词出现了语义的膨胀。
>
> 但我还是更喜欢精确的、不会引起歧义的说法：开发 *Web* 应用就叫 *Web 开发*，开发 *API* 就叫 *API 开发*。不过考虑到社区更习惯使用 *前后端*，因此本文会保留这种用法。
...
大多数浏览器都提供了 **DevTools**，这是一个开发者调试面版，内容非常丰富。几个比较常用的功能是 *元素*、*控制台*、*网络*、*源代码*。另外一些浏览器插件也会为 *DevTools* 提供额外的功能。
...
随着技术的发展，用 Web 技术开发 *Native/Desktop 应用* 出现了新的方式——在应用中调用系统自带的 *WebView*。有些人把这种新的应用形态叫做 *Hybrid 应用*，即混合应用。这需要系统本身支持 *WebView*。

随着这些尝试逐渐成熟，前端也许会走向统一。至于为什么是 *Web* 统一前端，我想这可能是因为它有着无与伦比的生态吧。

### 更多 Web 技术

Web 开发者总是需要学习新东西，这不仅因为框架、库、工具等升级得很快，就连底层的技术也发展得很快。很多内容无法在这里说清 ~~其实是很多技术我也没深入研究过~~ ，你可以自行研究
...
部分 JavaScript/TypeScript 库/工具因为更加通用，并非专属于 Web，所以没有列举在这里，你可以阅读 [JavaScript](../编程语言/JavaScript.md) 章节了解更多。
...
- Tauri 通过 WebView 来用 Web 技术开发开发 *桌面应用* 的框架
- Cordova 通过 WebView 来用 Web 技术开发开发 *移动应用* 的框架
...
- miniserve 静态文件服务器，由 Rust 编写，没有外部运行时依赖。启动时加个 `-u` 参数就可以生成二维码，很方便在手机上体验应用，不用再自己查询 IP 并手动输入了
