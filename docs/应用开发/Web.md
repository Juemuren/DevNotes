...
> [!Note]- 前端和后端
>
> 现代 *Web* 应用通常都会使用 **前后端分离** 的设计：前端实现需要的 **UI**，后端实现需要的 **API**。但我不是很想把后端开发归类为 Web 开发，因为后端实现的 API 不仅可以用于 *Web* 应用，也可以用于别的应用。
>
> 另外，我其实不是很喜欢 *前端* 和 *后端* 这种说法，因为这两个词在计算机相关领域已经被严重滥用了。暂且不说编译器、集成电路等地方也再使用前后端这种术语，单单 Web 方面前端一词就非常模糊：有时前端几乎就是 Web 的同义词，有时用来指代 UI，有时涵义又更接近客户端。当然这可能是因为 Web 技术发展得太快，而人们改变习惯用语需要时间，才导致前端一词出现了语义的膨胀。
>
> 但我还是更喜欢精确的、不会引起歧义的说法：开发基于 *Web* 的应用就叫 *Web 开发*，开发需要的 *API* 就叫 *API 开发*。不过考虑到社区习惯使用这些词语，因此本文会保留这种用法。
...
随着 Web 生态逐渐发展，这些框架越来越强大，其中一些渐渐不满足于只制作 *Web* 应用了，开始涉足别的领域。
