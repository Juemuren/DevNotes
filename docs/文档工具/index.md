...
- [Chrome](../网络工具/Chrome.md) 虽然是浏览器，但提供了 Headless 模式，可以用脚本批量将 HTML 打印为 PDF
...
- [bat](../数据处理工具/Bat.md) 更现代的 cat，也能用于在终端阅读 Markdown，但功能上远不止于 Markdown
...
- [PDFcpu](PDFcpu.md) 一个可以对 PDF 文档进行各种处理的命令行工具。类似的还有 `qpdf`、`xpdf`、`pdftk`、`poppler`、`GhostScript`
...
> - [Markdown Footnotes](https://marketplace.visualstudio.com/items?itemName=bierner.markdown-footnotes) 为 Markdown 提供脚注功能
...
> [!Note]- VSCode 扩展
> 一些 VSCode 扩展也能提供拼写检查功能。尽管没有命令行工具的版本，但也归类到拼写检查工具中
>
...
- VitePress 通过 Vite 和 Vue 驱动，样式很现代
